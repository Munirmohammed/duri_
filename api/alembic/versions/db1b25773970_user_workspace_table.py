...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column("membership", sa.String(), nullable=True, server_default="contributor", comment="membership type to that workspace, ie contributor | admin"), # user | admin
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=func.now()),
    )
    op.create_table(
        "user_team",
        sa.Column("user_id", UUID(as_uuid=True),  sa.ForeignKey('user.id'), primary_key=True, nullable=False), # *(required)
        sa.Column("workspace_id", UUID(as_uuid=True),  sa.ForeignKey('workspace.id'), primary_key=True, nullable=False), # *(required)
        sa.Column("team_id", UUID(as_uuid=True),  sa.ForeignKey('team.id'), primary_key=True, nullable=False),
